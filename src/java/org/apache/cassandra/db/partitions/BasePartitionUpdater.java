...
public class BasePartitionUpdater implements ColumnData.PostReconciliationFunction
{
    ...
    protected void makeContextAwareCloner(int estimatedCloneSize)
    {
        adjustUnusedContextAwareCloner();
        contextCloner = cloner.createContextAwareCloner(estimatedCloneSize);
    }

    protected void adjustUnusedContextAwareCloner()
    {
        if (contextCloner != null && contextCloner != cloner)
            contextCloner.adjustUnused();
    }

    protected void useNonContextAwareCloner()
    {
