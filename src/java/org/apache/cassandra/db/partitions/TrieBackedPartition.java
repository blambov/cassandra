...
public class TrieBackedPartition implements Partition
{
    ...
    protected static InMemoryDTrie<Object> build(UnfilteredRowIterator iterator)
    {
        try
        {
    ...
    Iterator<Row> rowIterator(Trie<Object> trie, Direction direction)
    {
        ...
        return Iterators.transform(trie.filteredEntryIterator(direction, RowData.class),
     ...
     */
    protected static void putInTrie(ClusteringComparator comparator, boolean useRecursive, InMemoryDTrie<Object> trie, Row row) throws TrieSpaceExhaustedException
    {
     ...
     */
    protected static void putStaticInTrie(ClusteringComparator comparator, InMemoryDTrie<Object> trie, Row staticRow) throws TrieSpaceExhaustedException
    {
    ...
    public int rowCount()
    {
        ...
        // Note: InMemoryDTrie.valuesCount() is not good enough here as values will remain in the trie if overwritten
        // with forceCopy, and we may be holding a Memtable tail trie which only has a portion of the values.
        return Iterables.size(trie.filteredValues(RowData.class));
    }
    ...
    public Iterator<Row> rowIterator()
    {
        ...
        final Trie<Object> nonStaticTrie = trie.subtrie(path(ClusteringBound.BOTTOM), null);
        return rowIterator(nonStaticTrie, Direction.FORWARD);
    }
    ...
    public UnfilteredRowIterator unfilteredIterator(ColumnFilter selection, NavigableSet<Clustering<?>> clusteringsInQueryOrder, boolean reversed)
    {
        ...
        Iterator<Row> rowIter = new AbstractIterator<Row>() {
            ...
            protected Row computeNext()
            {
                ...
                {
                    Row row = getRow(clusterings.next());
                    if (row != null)
                        return row;
                }
                return endOfData();
     ...
     */
    public static final InMemoryDTrie.UpsertTransformer<Object, Object> NO_CONFLICT_RESOLVER =
            (existing, update) ->
            {
                if (existing != null)
    ...
    public static class ContentBuilder
    {
        ...
        private final InMemoryDTrie<Object> trie;
        ...
        public ContentBuilder(TableMetadata metadata, DeletionTime partitionLevelDeletion, boolean isReverseOrder)
        {
            ...
            this.deletionBuilder = MutableDeletionInfo.builder(partitionLevelDeletion,
                                                               comparator,
                                                               isReverseOrder);
            this.trie = InMemoryDTrie.shortLived();
        ...
        public InMemoryDTrie<Object> buildTrie() throws TrieSpaceExhaustedException
        {
