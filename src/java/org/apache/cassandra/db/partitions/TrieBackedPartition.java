...
public class TrieBackedPartition implements Partition
{
    ...
    public static class RowData
    {
        ...
        // TODO track minTimestamp to avoid deletions doing anything
     ...
     */
    protected static void putInTrie(ClusteringComparator comparator, InMemoryDeletionAwareTrie<Object, TrieTombstoneMarker> trie, Row row) throws TrieSpaceExhaustedException
    {
        ...
        if (!row.isEmptyAfterDeletion())
        {
            trie.apply(DeletionAwareTrie.<Object, TrieTombstoneMarker>singleton(comparator.asByteComparable(clustering),
                                                                                ...
                                                                                rowToData(row)),
                       NO_CONFLICT_RESOLVER,
                       NO_CONFLICT_RESOLVER,
                       (data, deletion) -> data, // assume no conflict
                       BIFUNCTION_THROW,
                       true,
                       x -> false);
        }
    ...
    {
        // TODO: Tell the trie not to go over the live path.
    ...
    {
        // TODO: Can static rows be tombstoned?
    ...
    {
        // TODO: Keep a count of deletions too.
    ...
    public Row getRow(Clustering clustering, ByteComparable path)
    {
        ...
        if (data != null)
            return data.toRow(clustering, marker.deletionTime());
        else if (marker != null)
     ...
     */
    public static final InMemoryTrie.UpsertTransformer NO_CONFLICT_RESOLVER =
            (existing, update) ->
            {
                if (existing != null)
    ...
    public static final BiFunction BIFUNCTION_THROW = (x, y) ->
            {
                throw new AssertionError("No self-deletion expected.");
            };
    ...
    public static class ContentBuilder
    {
        ...
        public ContentBuilder complete() throws TrieSpaceExhaustedException
        {
            ...
            trie.putRecursive(ByteComparable.EMPTY, PARTITION_MARKER, NO_CONFLICT_RESOLVER);    // will throw if called more than once
            return this;
        }
