...
public class TrieMemtable extends AbstractAllocatorMemtable
{
    ...
    static Trie<Object> trieFor(PartitionUpdate update)
    {
        ...
        if (!staticRow.isEmpty())
            trie = trie.mergeWith(Trie.singleton(update.metadata().comparator.asByteComparable(Clustering.STATIC_CLUSTERING), staticRow), Trie.throwingResolver());
    ...
    {
        return trieFor(update).prefix(update.partitionKey());
    }
        ...
        public long put(PartitionUpdate update, UpdateTransaction indexer, OpOrder.Group opGroup)
        {
            ...
            try
            {
                try
                {
                    ...
                    try
                    {
                        data.apply(asMergableTrie(update),
                                   updater,
