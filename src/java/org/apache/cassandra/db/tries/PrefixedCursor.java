...
abstract class PrefixedCursor<T, C extends Cursor<T>> extends DepthAdjustedCursor<T, C>
{
    ...
    PrefixedCursor(int firstPrefixByte, ByteSource prefix, C tail)
    {
        ...
        tail.assertFresh();
        setPositionAndCheckPrefixDone(tail.encodedPosition() & ~Cursor.FLAGS_MASK); // initial position with the correct direction
    }
    ...
    implements DeletionAwareCursor<T, D>
    {
        ...
        public RangeCursor<D> deletionBranchCursor(Direction direction)
        {
            if ((currentPosition & MAY_HAVE_DELETION_BRANCH_BIT) != 0 && deletionBranch == null)
            {
                setPositionAndCheckPrefixDone(Cursor.rootPosition(direction));
            }
