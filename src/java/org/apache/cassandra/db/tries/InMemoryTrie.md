...
- `InMemoryTrie.longLived(OpOrder)` creates a trie which is expected to grow large, to remain in place for a long time,
  ...
  `MemtableSubrange` (and in general database memtables) use long-lived tries, with the table's `readOrdering` (which all
...
- (2) the procedure may fail before the attachment point and the old cell may remain reachable even for
  this thread;
- (3) it may still be needed by the mutator (e.g. a multi cell is freed when we recognize that the last node in the
...
For efficiency the allocation strategy does not work with individual cells, but rather in blocks of of 1024. Newly
