...
public interface VerificationCursor
{
    ...
    /// - exhausted state matches `Cursor.EXHAUSTED_POSITION`
    /// - start state matches `Cursor.Cursor.rootPosition(direction)`
    class Plain<T, C extends Cursor<T>> implements Cursor<T>, Cursor.TransitionsReceiver
    {
        ...
        public long advanceMultiple(Cursor.TransitionsReceiver receiver)
        {
            ...
            int returnedDepth = Cursor.depth(returnedPosition);
            assert !advanceMultipleCalledReceiver || depth == returnedDepth + 1 :
        ...
        private void verifySkipRequest(long encodedSkipPosition)
        {
            ...
            int returnedDepth = Cursor.depth(returnedPosition);
            assert skipDepth <= returnedDepth + 1 :
