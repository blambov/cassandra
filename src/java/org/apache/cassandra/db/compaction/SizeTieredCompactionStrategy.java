...
public class SizeTieredCompactionStrategy extends AbstractCompactionStrategy
{
    ...
    public Collection<AbstractCompactionTask> getNextBackgroundTasks(long gcBefore)
    {
        ...
        while (true)
        {
            ...
            if (transaction != null)
                return Collections.singletonList(new CompactionTask(cfs, scannerFactory(), transaction, gcBefore));
    ...
    public synchronized List<AbstractCompactionTask> getMaximalTasks(final long gcBefore, boolean splitOutput)
    {
        ...
        if (txn == null)
            return null;
        if (splitOutput)
            return Arrays.<AbstractCompactionTask>asList(new SplittingCompactionTask(cfs, scannerFactory(), txn, gcBefore));
        return Arrays.<AbstractCompactionTask>asList(new CompactionTask(cfs, scannerFactory(), txn, gcBefore));
    }
    ...
    public AbstractCompactionTask getUserDefinedTask(Collection<SSTableReader> sstables, final long gcBefore)
    {
        ...
        return new CompactionTask(cfs, scannerFactory(), transaction, gcBefore).setUserDefined(true);
    }
    ...
    {
        public SplittingCompactionTask(ColumnFamilyStore cfs, ScannerFactory scannerFactory, ILifecycleTransaction txn, long gcBefore)
        {
            super(cfs, scannerFactory, txn, gcBefore);
        }
