    ...
    protected long advanceNode()
    {
        ...
        while (true)
        {
            ...
            if (beyondLimit)
            {
                ...
                if (payloadFlags() != 0)
                {
                    // If we are fully inside the covered space (stackTop.limit == NOT_AT_LIMIT), report.
                    // On the exact match of the limit, stackTop.limit == END_OF_STREAM, so only report that if we are admitting exact matches
                    // (An exact match is also acceptable for ADMIT_PREFIXES, but we prefer to process that in the else-if case to also reset reportingPrefixes.)
