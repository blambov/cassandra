...
public class CursorTest
{
    ...
    {
        qt().forAll(DEPTH_GEN, DEPTH_GEN, TRANSITION_GEN, DIRECTION_GEN)
    ...
    public void testUndecodedTransition()
    {
        // Test forward direction
        long pos = Cursor.encode(1, 0x12, Direction.FORWARD);
        assertEquals(0x12 << 1, Cursor.undecodedTransition(pos));

        // Test reverse direction (should have bit 0x100 set)
        pos = Cursor.encode(1, 0x12, Direction.REVERSE);
        assertEquals((0x12 ^ 0xFF) << 1, Cursor.undecodedTransition(pos));
        ...
        for (int i = 0; i < 0x100; i++)
        {
            // Forward direction
            pos = Cursor.encode(1, i, Direction.FORWARD);
            assertEquals(i << 1, Cursor.undecodedTransition(pos));

            // Reverse direction
            pos = Cursor.encode(1, i, Direction.REVERSE);
            assertEquals((i ^ 0xFF) << 1, Cursor.undecodedTransition(pos));
        }
    }
    ...
    {
        qt().forAll(DEPTH_GEN, TRANSITION_GEN, DIRECTION_GEN)
    ...
    {
        qt().forAll(DEPTH_GEN, TRANSITION_GEN, TRANSITION_GEN, DIRECTION_GEN)
        ...
        qt().forAll(DEPTH_GEN, TRANSITION_GEN, DIRECTION_GEN)
            .checkAssert((depth, transition, direction) -> {
                ...
                if (transition.intValue() != direction.select(0xFF, 0x00))
                    ...
                    assertEquals(0x200, Cursor.undecodedTransition(newPos));
                assertEquals(direction, Cursor.direction(newPos));
            });
    }
}
