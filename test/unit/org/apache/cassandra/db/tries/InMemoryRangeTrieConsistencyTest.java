...
public class InMemoryRangeTrieConsistencyTest extends ConsistencyTestBase<InMemoryRangeTrieConsistencyTest.Content, RangeTrie<InMemoryRangeTrieConsistencyTest.Content>, InMemoryRangeTrie<InMemoryRangeTrieConsistencyTest.Content>>
{
    @Override
    InMemoryRangeTrie<Content> makeTrie(OpOrder readOrder)
    {
    ...
    @Override
    RangeTrie<Content> makeSingleton(ByteComparable b, Content content)
    {
        return RangeTrie.singleton(b, VERSION, content);
    }

    @Override
    RangeTrie<Content> withRootMetadata(RangeTrie<Content> wrapped, Content metadata)
    {
        return wrapped.mergeWith(RangeTrie.singleton(ByteComparable.EMPTY, VERSION, metadata), Trie.throwingResolver());
    }

    @Override
    RangeTrie<Content> merge(Collection<RangeTrie<Content>> tries, Trie.CollectionMergeResolver<Content> mergeResolver)
    {
        return RangeTrie.merge(tries, mergeResolver);
    }

    @Override
    void apply(InMemoryRangeTrie<Content> trie, RangeTrie<Content> mutation, InMemoryBaseTrie.UpsertTransformer<Content, Content> mergeResolver, Predicate<InMemoryBaseTrie.NodeFeatures<Content>> forcedCopyChecker) throws TrieSpaceExhaustedException
    {
    ...
    @Override
    void printStats(InMemoryRangeTrie<Content> trie, Predicate<InMemoryBaseTrie.NodeFeatures<Content>> forcedCopyChecker)
    {
    ...
    static abstract class Content implements RangeMarker<Content>
    {
        final String pk;
    ...
    static class Value extends Content
    {
        ...
        @Override
        public Content precedingState(Direction direction)
        {
            return null;
        }

        @Override
        public Content branchState()
        {
            return new CoveredValue(pk, ck, value, seq);
        }
    ...
    static class CoveredValue extends Value
    {
        CoveredValue(String pk, String ck, int value, int seq)
        {
            super(pk, ck, value, seq);
        }
