...
import static org.apache.cassandra.db.tries.TestRangeMarker.verify;
...
public class RangeTrieMergeTest
{
    ...
    private List<TestRangeMarker> deletedRanges(ByteComparable... dataPoints)
    {
        ...
        List<TestRangeMarker> markers = new ArrayList<>();
    ...
    private List<TestRangeMarker> getTestRanges()
    {
    ...
    private void testMerges()
    {
        ...
        List<TestRangeMarker> set1 = deletedRanges(null, of(24), of(25), of(29), of(32), null);
        List<TestRangeMarker> set2 = deletedRanges(of(14), of(17),
                                                   of(22), of(27),
                                                   of(28), of(30),
                                                   of(32), of(34),
                                                   of(36), of(40));
        List<TestRangeMarker> set3 = deletedRanges(of(17), of(18),
                                                   of(19), of(20),
                                                   of(21), of(22),
                                                   of(23), of(24),
                                                   of(25), of(26),
    ...
    private void testMerges(List<TestRangeMarker> set1, List<TestRangeMarker> set2, List<TestRangeMarker> set3)
    {
...
//        testCollectionMerge(message + " collection", Lists.newArrayList(fromList(testRanges)), testRanges, sets);
    ...
    public void testMerge(String message, RangeTrie<TestRangeMarker> trie, List<TestRangeMarker> merged, List<TestRangeMarker>... sets)
    {
        ...
        else
        {
            for (int toRemove = 0; toRemove < sets.length; ++toRemove)
            {
                ...
                testMerge(message + " " + toRemove,
                          trie.mergeWith(fromList(ranges), TestRangeMarker::combine),
    ...
    InMemoryRangeTrie<TestRangeMarker> duplicateTrie(RangeTrie<TestRangeMarker> trie)
    {
        try
        {
    ...
    public void testMergeToInMemoryTrie(String message, InMemoryRangeTrie<TestRangeMarker> trie, List<TestRangeMarker> merged, List<TestRangeMarker>... sets)
    {
        System.out.println("Markers: " + merged);
        verify(merged);
    ...
    TestRangeMarker upsertMarkers(TestRangeMarker left, TestRangeMarker right)
    {
        if (left == null)
            return right;
        if (right == null)
            return left;
        return TestRangeMarker.combine(left, right);
    }

    public void testCollectionMerge(String message, List<RangeTrie<TestRangeMarker>> triesToMerge, List<TestRangeMarker> merged, List<TestRangeMarker>... sets)
    {
        System.out.println("Markers: " + merged);
        verify(merged);
        // Test that intersecting the given trie with the given sets, in any order, results in the expected list.
        ...
        if (sets.length == 0)
        {
            RangeTrie<TestRangeMarker> trie = RangeTrie.merge(triesToMerge, TestRangeMarker::combineCollection);
            try
            {
                assertEquals(message + " forward b" + bits, merged, toList(trie, Direction.FORWARD));
                assertEquals(message + " reverse b" + bits, Lists.reverse(merged), toList(trie, Direction.REVERSE));
    ...
    TestRangeMarker delete(int deletionTime, TestRangeMarker marker)
    {
        if (deletionTime < 0)
            return marker;
    ...
    List<TestRangeMarker> mergeLists(List<TestRangeMarker> left, List<TestRangeMarker> right)
    {
        int active = -1;
        Iterator<TestRangeMarker> rightIt = right.iterator();
        TestRangeMarker nextRight = rightIt.hasNext() ? rightIt.next() : null;
        List<TestRangeMarker> result = new ArrayList<>();
        for (TestRangeMarker nextLeft : left)
        {
            while (true)
            {
                int cmp;
                ...
                if (cmp == 0)
                {
                    TestRangeMarker processed = TestRangeMarker.combine(nextRight, nextLeft).toContent();
