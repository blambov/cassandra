...
public class RandomizedCancelCompactionsTest extends CQLTester
{
    ...
    {
        CassandraRelevantProperties.CESSATION_WAIT_SECONDS.setInt(5);
    ...
    public void testRandomizedCancellation() throws Exception
    {
        ...
        try
        {
            ...
            Future<?> taskCreator = executor.submit(() -> {
                while (testRunning.get() && System.currentTimeMillis() < endTime)
                {
                    try
                    {
                        ...
                        if (activeTasks < MAX_CONCURRENT_COMPACTIONS && cfs.getCompactionStrategyManager().isActive())
                        {
                        ...
                        if (cfs.runWithCompactionsDisabled(
                            () -> {
                                ...
                                assertEquals("No sstables intersecting range may be compacting inside runWithCompactionsDisabled block",
                                             0, cfs.getTracker().getCompacting().stream().filter(predicate).count());
                                return true;
                            },
                            predicate,
            ...
            List<CompactionInfo.Holder> remainingOps = CompactionManager.instance.active.getCompactions()
                                                                                        .stream()
                                                                                        .filter(op -> op.getCompactionInfo().getTable().orElse("unknown").equalsIgnoreCase(cfs.name))
                                                                                        .collect(Collectors.toList());
            
            assertEquals("No test operations should remain in ActiveOperations", 0, remainingOps.size());

            assertEquals("No sstables should be still compacting", 0, cfs.getTracker().getCompacting().size());
            ...
            assertTrue("The majority of the cancellations should succeed", cancellationAttempts.get() < cancellationSuccess.get() * 2);
        }
        finally
        {
            testRunning.set(false);
    ...
    private static class RandomizedCompactionTask extends AbstractCompactionTask
    {
        ...
        protected int executeInternal(ActiveCompactionsTracker activeCompactions)
        {
            try
            {
                ...
                ci = new CompactionIterator(transaction.opType(), scanners, controller, FBUtilities.nowInSeconds(), TimeUUID.Generator.nextTimeUUID());
                ...
                while (slept < sleepTimeMs)
                {
                    ...
                    Uninterruptibles.sleepUninterruptibly(Math.min(100, sleepTimeMs - slept), TimeUnit.MILLISECONDS);
                    slept += 100;
                }
            ...
            {
                complete(activeCompactions);
                finished.set(true);
            }
            return 0;
        }
        
        private void complete(ActiveCompactionsTracker activeCompactions)
        {
            cleanupRan.set(true);
